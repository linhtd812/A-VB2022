...
def load_ssl_model(model_path):
    ...
    elif 'data2vec' in model_path:
        model = transformers.Data2VecAudioModel.from_pretrained(model_path)
    elif 'dino' in model_path:
        # model = transformers.ViTFeatureExtractor.from_pretrained(model_path)
        model = transformers.ViTModel.from_pretrained(model_path)
    else: raise
    return model
