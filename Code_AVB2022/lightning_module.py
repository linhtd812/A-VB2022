...
class BaselineLightningModule(pl.LightningModule):
    ...
    def construct_model(self):
        self.feature_extractor = hydra.utils.instantiate(
            self.cfg.model.feature_extractor, cfg=self.cfg, _recursive_=False
        )
        self.model = hydra.utils.instantiate(
            self.cfg.model.model, cfg=self.cfg, _recursive_=False
        )
        print(self.model)
    ...
    #     save_path = "/mnt/1TSSD/AVB/ExVo/outputs/2022-08-22/15-50-56/"
    #     checkpoint_path = save_path + "pl_log" + "/epoch=36-step=184926.ckpt"
    #     ckpt = torch.load(checkpoint_path)
    #     state_dict = ckpt["state_dict"]

    #     # Remove chain layers from state_dict
    #     new_state_dict = {}
    #     for k, v in state_dict.items():
    #         if "model.chain" not in k and "model.squeeze_ln" not in k:
    #             new_state_dict[k] = v
    ...
    #     # Freeze HuBERT
    #     for k, v in self.feature_extractor.wav2vec2.named_parameters():
    #         v.requires_grad = False

    #     # Freeze GRU
    #     for k, v in self.model.gru.named_parameters():
    #         v.requires_grad = False
    ...
    #     # Freeze attention
    #     for k, v in self.model.alpha_ln.named_parameters():
    #         v.requires_grad = False

    #     for k, v in self.model.beta_ln.named_parameters():
    #         v.requires_grad = False
    #     print("232423")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out = {
            "gt_emotion": batch["emotion"].detach().cpu(),
            "pred_emotion": pred_emotion["pred_final"].detach().cpu(),
        }
    ...
    def predict_batch(self, batch):
        pred_emotion = self(batch)
        return pred_emotion["pred_final"].detach().cpu()


class BaselineLightningModule_Spec(pl.LightningModule):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.lr = cfg.train.lr
    ...
    def forward(self, batch):
        ...
        # import pdb; pdb.set_trace()
        feat, mel_ft = self.extract_feature(inputs, mel_spec)
        outputs = self.model(feat, mel_ft, batch)
        return outputs

    def extract_feature(self, inputs, mel_spec):
        # inputs: [#B, #seq_len]
        feat, mel_ft = self.feature_extractor(inputs, mel_spec)  # Missing mel_spec
...
class DALightningModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # collect results and metrics
        out = {
            "gt_emotion": gt_emotion.detach().cpu(),
            "pred_emotion": pred_emotion.detach().cpu(),
        }
        # out.update({k: v.detach().cpu() for k, v in loss_dict.items()})
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam(
            [
                {
                    "params": chain(
                        self.model.parameters(), self.criterion.parameters()
                    ),
