...
def evaluation_full_val(cfg):
    root = cfg.log_dir
    best_ckpt_path = "/mnt/1TSSD/AVB/ExVo/outputs/2022-08-03/16-17-09/pl_log/epoch=16-step=84966.ckpt"

    print(f"Load best ckpt paths from {best_ckpt_path}")
    best_ckpt_paths = torch.load(best_ckpt_path)

    best_ckpt_path = best_ckpt_paths
    save_dir = None
    cfg.dataset.train.csv_path = f"data/labels/filelists/exvo_train.csv"
    cfg.dataset.val.csv_path = f"data/labels/filelists/exvo_val.csv"
    eval_out = eval_single(cfg, save_dir, best_ckpt_path)
    print(eval_out)
...
def eval_single(cfg, save_dir, ckpt):
    ...
    out = {
        "woft_ccc": woft_ccc,
        "woft_ccc_single": woft_ccc_single,
        "ft_ccc": ft_ccc,
        "ft_ccc_single": ft_ccc_single,
    }
    return out
