...
def process_silence(src_dir, tgt_dir):
    ...
    print(
        f"Stats of durations before trmming: mean {before.mean()}s, std {before.std()}s, min {before.min()}s, max {before.max()}s"
    )
    print(
        f"Stats of durations after trmming: mean {after.mean()}s, std {after.std()}s, min {after.min()}s, max {after.max()}s"
    )
